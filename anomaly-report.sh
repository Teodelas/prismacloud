...
for i in "${!anomaly_reports[@]}"; do
  echo ${anomaly_reports[i]}
  IFS=':' read -r -a array <<< ${anomaly_reports[i]}
  REPORT_LOCATION="$HOME/reports/${array[0]}_report_$REPORT_DATE.csv"

  export POLICY_ID=${array[1]}
