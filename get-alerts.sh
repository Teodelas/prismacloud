...
echo "Id, Policy Name, Policy Type, Severity, Resource Name, Cloud Type, Account Id, Resource Id" > $REPORT_LOCATION
...
curl -L -X GET \
        ...
        -H "x-redlock-auth: $PC_JWT" | jq .items[] | jq -r '[.id, .policy.name, .policy.policyType, .policy.severity, .resource.name, .resource.cloudType, .resource.accountId, .resource.id] | @csv' >> $REPORT_LOCATION
